...
{
    public class RoleProviders : Controller
    {
        private ApiServices _api;
        private string _authorizationtoken;

        public RoleProviders(string token)
        {
        ...
        #region DELETE
        public async Task<AM_Role> Delete(string id)
        {
            bindApiServices();

            var result = await _api.Delete(id);
            return result != null ? JsonConvert.DeserializeObject<AM_Role>(result) : null;
        }
        ...
        {
            _api = new ApiServices("Roles", _authorizationtoken);
        }
    }
}